...
def main(eco_level, year):
    ...
    # TODO: Add to paths?
    lookup_table_path = RESULTS_DIR / f"HLH_Fits" /f"{year}-01-01" / "hazard_lookup.parquet"
...
def _get_parser():
    ...
    p.add_argument(
        "--eco_level", type=int, default=3, help="The eco regions level to use."
    )
    p.add_argument(
        "--year", type=int, help="The year to calculate burn probability"
    )
    return p
