...
def create_lookup_table(
    ...
    common_keys = list(
        filter(lambda x: x in FIXED_LABELS + VARIED_LABELS,
               fits.columns
        )
    )
    ...
    for row in fits.iter_rows(named=True):
        ...
        filters = {k:v for k,v in row.items() if k in FIXED_LABELS+VARIED_LABELS}
        pl_filters = [
            pl.col(col).is_in(vals) for col, vals in filters.items()
            ]
        combined_filter = pl_filters[0]
        for f in pl_filters[1:]:
            combined_filter = combined_filter & f
        unique_sts = np.concatenate([
        sts.filter(combined_filter).select(pl.col("st")).unique().sort(pl.col("st")).sort("st").to_numpy().flatten(),
