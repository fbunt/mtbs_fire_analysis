...
def get_dt_polygons(refresh=False):
    ...
    if refresh:
        dts = get_dts()
        dt_polygons = dts.group_by(["eco3","nlcd2","Event_ID1","Event_ID2","dt"]).agg(
            pl.len().alias("Pixel Count")
        ).filter(pl.col("dt") > 0.5)
        dt_polygons.write_parquet(CACHE_DIR / "dt_polygons.parquet")
    else:
        dt_polygons = pl.scan_parquet(CACHE_DIR / "dt_polygons.parquet").collect()
    return dt_polygons
...
def get_st_polygons(refresh=False):
    ...
    if refresh:
        ...
        st_polygons = pl.scan_parquet(CACHE_DIR / "st_polygons.parquet").collect()
    return st_polygons
...
def get_events(refresh=False):
    ...
    if refresh:
        lf = get_lf()
        events = build_event_histories(lf, max_date=None, fixed_pivots=["eco_lvl_3"],varied_pivots=["nlcd"])
