...
def do_fit(fitter, event_history, varied_filters, min_date, max_date, total_pixels):
    #(fitter, dt_polygons, st_polygons, num_pixels, def_st):
    _dts_and_counts = event_hist_to_dts(event_history,
        varied_filters)
    ...
    # )
    _cts_and_counts = event_hist_to_cts(event_history,
        max_date = max_date,
    ...
    _uts_and_counts = event_hist_to_uts(event_history,
        min_date = min_date,
        varied_filters = varied_filters
    )
    ...
    fitter.fit(_dts, _dt_counts, _cts, _ct_counts, _uts, _ut_counts,
               np.array([window_length]), np.array([empty_pixels]))

    return fitter, _dts, _dt_counts, _cts, _ct_counts

def bootstrap_resample(
    df: pl.DataFrame,
    cluster_col: Hashable
) -> pl.DataFrame:
    """
    ...
    sampled_keys = (
        ...
        .with_row_index("_draw") # distinguishes identical keys selected twice
    )
...
def run_fit_over_configs(event_histories, configs, min_date, max_date):
    ...
    for config in configs:
        ...
        fixed_filters = {k:v for k,v in config.items() if k in fixed_labels and k != "name"}
        varied_filters = {k:v for k,v in config.items() if k in varied_labels and k != "name"}
        fixed_pl_filters = [
        ...
        config_histories = event_histories.filter(
            combined_filter
        )

        total_pixels_filters = {k:v for k,v in config.items() if k != "name"}
        pl_filters = [
            pl.col(col).is_in(vals) for col, vals in total_pixels_filters.items()
        ]
        combined_filter = pl_filters[0]
        for f in pl_filters[1:]:
            combined_filter = combined_filter & f
        total_pixels = pix_counts.filter(combined_filter).get_column("Total num pixels").sum()
